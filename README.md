...
./rke up
```
(spam run ./rke up if fail <= 3-attempts and use empty passphrase in key-gen command, rke notoriously fails a ton: https://github.com/rancher/rke/issues/2632#issuecomment-914315247 so my new practice will be to establish a 1 node rke cluster, and add nodes into the cluster with associatyed roles via uncommenting them in the desired final cluster.yml with each rke up call made per node)      
